...
export function BidsPage({ apiCall }) {
  ...
  // Backend se API call karke asli bids lana
  useEffect(() => {
    if (apiCall) {
  ...
  // Winning amount backend se calculate karna
  ...
  return (
    <div className="bids-page screen">
      ...
      {loading ? (
         <div style={{textAlign:'center', padding:40, color:'#888'}}>⏳ Bids load ho rahi hain...</div>
      ) : bids.length === 0 ? (
         <div style={{textAlign:'center', padding:40, color:'#888'}}>📭 Aapne abhi tak koi bid nahi lagayi hai</div>
      ) : (
          ...
          {bids.map(b => {
            ...
            <div key={b.id} className="ml-item">
              <div className="ml-left">
                ...
                <div>
                  {/* Backend columns ke naam map kiye */}
...
export function TxnsPage({ apiCall, navigate }) {
  ...
  return (
    <div className="screen" style={{paddingBottom:80}}>
      {/* Header */}
      <div style={{
        background:'#0d2a1a', padding:'14px 16px', display:'flex', alignItems:'center', gap:12,
        borderBottom:'1px solid #1a4a2a', position:'sticky', top:0, zIndex:10
      }}>
        {navigate && (
        ...
        <button onClick={fetchTxns} style={{
          ...
          color:'#f0a500', padding:'6px 12px', borderRadius:8, cursor:'pointer', fontSize:12, fontWeight:700
        }}>🔄 Refresh</button>
      </div>
      ...
      <div style={{display:'flex', gap:8, padding:'12px 14px'}}>
        {[['all','All'], ['credit','Credit ⬆️'], ['debit','Debit ⬇️']].map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)} style={{
            ...
            fontWeight:700, fontSize:12, background: filter === val ? '#f0a500' : '#1a3a26',
            color: filter === val ? '#000' : '#aaa', transition:'all 0.15s'
          }}>{label}</button>
        ))}
      </div>
        ...
        {filtered.map((tx, i) => {
          ...
          return (
            <div key={tx.id || i} style={{
              background:'#1a2e22', borderRadius:10, padding:'14px 16px', marginBottom:10, display:'flex', alignItems:'center', gap:14,
              borderLeft:`3px solid ${credit ? '#22c55e' : '#ef4444'}`
            }}>
              <div style={{
                width:40, height:40, borderRadius:10, flexShrink:0,
                background: credit ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18
              }}>
                {credit ? '⬆️' : '⬇️'}
              </div>
              <div style={{flex:1, minWidth:0}}>
                <div style={{fontWeight:700, color:'#fff', fontSize:13, marginBottom:3}}>{typeLabel(tx.type)}</div>
                <div style={{fontSize:11, color:'#888', marginBottom:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
              ...
              <div style={{textAlign:'right', flexShrink:0}}>
                <div style={{ fontWeight:700, fontSize:16, color: credit ? '#22c55e' : '#ef4444' }}>
                  {credit ? '+' : '-'}₹{amount.toLocaleString('en-IN', {minimumFractionDigits:2})}
                </div>
                {balAfter !== null && (
                  <div style={{fontSize:10, color:'#555', marginTop:3}}>Bal: ₹{Number(balAfter).toLocaleString('en-IN')}</div>
                )}
                {(tx.statusTxt || tx.status) && (
                  <div style={{
                    fontSize:9, fontWeight:700, marginTop:4, padding:'2px 6px', borderRadius:4, display:'inline-block',
                    background: (tx.statusTxt||tx.status) === 'SUCCESS' || (tx.statusTxt||tx.status) === 'approved' ? 'rgba(34,197,94,0.2)' : 'rgba(240,165,0,0.2)',
                    color: (tx.statusTxt||tx.status) === 'SUCCESS' || (tx.statusTxt||tx.status) === 'approved' ? '#22c55e' : '#f0a500'
                  }}>
      ...
      {!loading && filtered.length > 0 && (
        <div style={{textAlign:'center', padding:'10px 0 20px', fontSize:11, color:'#555'}}>{filtered.length} transactions dikh rahi hain</div>
      )}
    </div>
  );
}
...
export function SupportPage({ apiCall, user }) {
  ...
  const [profilePicPreview, setProfilePicPreview] = useState(
    user?.profile_pic || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}&background=0d3526&color=f0a500&size=150`
  );
  const [loading, setLoading] = useState(false);
  const [successMsg, setSuccessMsg] = useState('');
  const [errorMsg, setErrorMsg]     = useState('');

  // Fetch admin settings for support contacts
  useEffect(() => {
    if (!apiCall) return;
  ...
  // ── FIXED: updateProfile — correct API routes ──
  const updateProfile = async () => {
    setSuccessMsg('');
    setErrorMsg('');

    if (!profileForm.username.trim()) {
      setErrorMsg('❌ Username dalna zaruri hai!');
      return;
    }
    ...
    try {
      // ── Step 1: Update Name ──
      const profileRes = await apiCall('/api/auth/update-profile', 'PUT', {
        name: profileForm.username.trim()
      });

      if (!profileRes?.success) {
        setErrorMsg(profileRes?.message || '❌ Profile update fail ho gaya');
        setLoading(false);
        return;
      }

      // ── Step 2: Update Password (only if user filled it) ──
      if (profileForm.newPassword) {
        if (!profileForm.oldPassword) {
          setErrorMsg('❌ Purana password zaruri hai');
          setLoading(false);
          return;
        }
        ...
        if (profileForm.newPassword.length < 4) {
          setErrorMsg('❌ Naya password minimum 4 characters ka hona chahiye');
          setLoading(false);
          return;
        }

        const passRes = await apiCall('/api/auth/update-password', 'POST', {
          oldPassword: profileForm.oldPassword,
          newPassword: profileForm.newPassword
        });

        if (!passRes?.success) {
          setErrorMsg(passRes?.message || '❌ Password update fail ho gaya');
          setLoading(false);
          return;
        }
      }
      ...
      // Update avatar display with new name
      setProfilePicPreview(
        `https://ui-avatars.com/api/?name=${encodeURIComponent(profileForm.username.trim())}&background=0d3526&color=f0a500&size=150`
      );
  ...
  return (
    <div className="screen" style={{ paddingBottom: 80 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: '#f0a500', fontFamily: 'Rajdhani, sans-serif' }}>
          👤 My Profile
        </span>
      </div>
      ...
      {/* ── PROFILE CARD ── */}
      <div style={{
        background: 'linear-gradient(135deg, #0d2a1a 0%, #1a3a26 100%)',
        margin: 16,
        borderRadius: 14,
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', display: 'inline-block' }}>
            <img
              ...
              style={{
                ...
                border: '3px solid #f0a500', objectFit: 'cover',
                display: 'block'
              }}
            />
          </div>
          ...
          <div style={{
            ...
            borderRadius: 20, padding: '3px 12px', fontSize: 11, color: '#22c55e', fontWeight: 700
          }}>
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{
            ...
            <div style={{ ...labelStyle, color: '#f0a500' }}>🔐 Change Password (Optional)</div>
      ...
      <div style={{ padding: '0 16px', marginTop: 4 }}>
        ...
        <div
          onClick={() => window.open(`https://wa.me/91${contacts.phone}`, '_blank')}
          style={{
            cursor: 'pointer', display: 'flex', alignItems: 'center',
            background: '#0d2a1a', padding: '16px', borderRadius: 10,
            marginBottom: 10, border: '1px solid #1a4a2a'
          }}
        >
          <div style={{ fontSize: 28, marginRight: 16 }}>💬</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, color: '#fff', fontSize: 15 }}>WhatsApp Support</div>
            <div style={{ fontSize: 12, color: '#aaa', marginTop: 3 }}>+91 {contacts.phone}</div>
          </div>
          <div style={{ color: '#f0a500', fontSize: 22 }}>›</div>
        </div>

        <div
          onClick={() => window.open(`https://t.me/${contacts.telegram}`, '_blank')}
          style={{
            cursor: 'pointer', display: 'flex', alignItems: 'center',
            background: '#0d2a1a', padding: '16px', borderRadius: 10,
            border: '1px solid #1a4a2a'
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, color: '#fff', fontSize: 15 }}>Telegram Support</div>
      ...
      <div style={{
        ...
        <strong style={{ color: '#666' }}>MatkaKing</strong> · Version 5.0.0<br />
